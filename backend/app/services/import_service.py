...
class ImportService:
    ...
    def import_project_excel(db: Session, file_content: bytes, user_id: int):
        ...
        try:
            ...
            # Roll back any pending task inserts AND delete the just-created project
            # so a partial import doesn't leave an orphaned project around.
            logger.exception("Excel import failed mid-way; rolling back project %s", project_id)
            db.rollback()
